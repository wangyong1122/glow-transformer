...
class TransformerIns(Transformer):
    ...
    def forward(self, batch, mode='train', reverse=True, dataflow=['src', 'trg'], step=None):
        ...
        # print(source_inputs.size(), source_outputs.size(), target_inputs.size(), target_outputs.size())
        ...
        if mode == 'path':
            ...
            if self.order_mode == 'random':
                ...
                if self.args.sample_order:
                    targets, weights = self.particle_filtering(sources, targets, self.args.beta, resampling=self.args.resampling, gamma=info['L@G'])
                else:
            ...
            if target_inputs.dim() == 3:  # replicate 
                ...
                if weights is not None:
                    info['L@W'] = (weights * torch.log(weights + TINY)).mean()
    ...
    def particle_filtering(self, sources, targets, width=2, field='trg', resampling=False, gamma=1): 
        ...
        for t in range(T - 1):
            ...
            # get together and select top K words/positons
            topk2_logps = (word_logps * gamma + pointer_logps) * mask_trg - INF * (1 - mask_trg)  # B x W x T
            topk2_logps = topk2_logps * (eos_yet[:, :, None].float() * eos_mask[:, :, :-t] + 1 - eos_yet[:, :, None].float())
            # topk2_logqs = log_softmax(topk2_logps)  # B x W x T-t
            # topk2_logqs = topk2_logqs * (eos_yet[:, :, None].float() * eos_mask[:, :, :-t] + 1 - eos_yet[:, :, None].float())
            
            # :: sample at each beam saperately (Gumbel-softmax trick)::
            if (not self.args.resample_fix) or (not resampling):
                topk_inds = (topk2_logps + (topk2_logps.new_zeros(topk2_logps.size()).uniform_().add_(TINY).log_().neg_().add_(TINY).log_().neg_())).max(2)[1]
            else:
                K = min(W, topk2_logps.size(2))
                topk_inds = (topk2_logps + (topk2_logps.new_zeros(topk2_logps.size()).uniform_().add_(TINY).log_().neg_().add_(TINY).log_().neg_())).topk(K, 2)[1]
                
                if K < W:
                    topk_inds = torch.cat([topk_inds for _ in range(W)], 2)[:, :, :W] # make sure we have "W" candidates AT LEAST

            # whatever what you selected, the importance weights are the same here!! --- 
            weights = weights * torch.sum(torch.exp(topk2_logps), 2)
            weights = weights / weights.sum(1, keepdim=True)

            if not resampling:
                selected_pos = cand_pos.gather(2, topk_inds[:, :, None]).squeeze(-1)
                selected_trg = cand_trg.gather(2, topk_inds[:, :, None]).squeeze(-1)
                outs[:, :, t+1] = selected_trg
                poss[:, :, t+1] = selected_pos
                relation[:, :, :t+2, :t+2] = (poss[:, :, :t+2, None] - poss[:, :, None, :t+2]).clamp(-1, 1)

                # gather and update candidates
                unselected = (1 - cand_pos.new_zeros(B, W, T-t).scatter_(2, topk_inds[:, :, None], 1)).byte()
                cand_pos = cand_pos[unselected].view(B, W, T-t-1)
                cand_trg = cand_trg[unselected].view(B, W, T-t-1)
                mask_trg = mask_trg[:, :, 1:]
                ...
                def stratified_resample(weights):
                    ...
                    """
                    positions = (weights.new_zeros(weights.size()).uniform_() + torch.arange(W, device=weights.get_device())[None, :].float()) / W
                ...
                topk_beam_inds = stratified_resample(weights)

                # stop re-sampling flag:
                flag = (mask_trg.sum(2) >= W).long()
                topk_beam_inds = topk_beam_inds * flag + torch.arange(W, device=topk_beam_inds.get_device())[None, :] * (1 - flag)

                # if self.args.local_rank == 0:
                #     print('wei', weights[-1:])
                #     print('ind', topk_inds[-1:])
                #     print('bem', topk_beam_inds[-1:])


                if not self.args.resample_fix:
                    topk_cand_inds = topk_inds.gather(1, topk_beam_inds)
                
                else:
                    # fixing resampling: avoid to sample the same words 
                    L = topk_beam_inds.new_zeros(B, W).long()
                    topk_cand_inds = []
                    for k in range(W):
                        topk_cand_inds.append(topk_inds.gather(2, L[:, :, None]).squeeze(2).gather(1, topk_beam_inds[:, k:k+1]))
                        L.scatter_(1, topk_beam_inds[:, k:k+1], L.gather(1, topk_beam_inds[:, k:k+1])+1)
                    topk_cand_inds = torch.cat(topk_cand_inds, 1)


                    # if self.args.local_rank == 0:
                    #     print('can', topk_cand_inds[-1:])

                # reset weights
                weights = weights * 0 + 1 / W
                eos_yet = eos_yet.gather(1, topk_beam_inds)
