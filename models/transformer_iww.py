...
class TransformerIww(TransformerIns):
    ...
    def supervised_training(self, sources, targets):
        ...
        gumbel_noise = inference_attention_scores.new_zeros(inference_attention_scores.size()).uniform_().add_(TINY).log_().neg_().add_(TINY).log_().neg_().detach()
    ...
    # def enhanced_beam_search(self, encoding=None, mask_src=None, width=2, alpha=0.6, T=None, field='trg'):  # width: beamsize, alpha: length-norm
    #     assert self.word_first, 'currently word-first is in default.'

    #     encoding = self.decoder.prepare_encoder(encoding)

    #     W = 4
    #     V = self.io_dec.out.weight.size(0)
    ...
    #         # pointing to all locations for the whole vocabulary
    #         query, key1, key2 = pointer_query, pointer_keys[0][:, :, :t+1], pointer_keys[1][:, :, :t+1]
    #         pointer_scores = self.pointing(query.view(B*W, -1, C), key1.view(B*W, -1, C), key2.view(B*W, -1, C), output=False, embed_bias=True).view(B, W, -1, 2*t+2)  # B x W x V x (2t+2)

    #         # predict word, added with position's likelihood
    #         # topk2_logps: scores, topk2_inds: top word index at each beam, batch x beam x beam
    #         topk2_logps = log_softmax(self.io_dec.o(predictor))
    #         topk2_logps[:, :, self.fields[field].vocab.stoi['<pad>']] = -INF

    #         topk2_logps = (topk2_logps[:, :, :, None] + log_softmax(pointer_scores)).view(B, W, -1)
    #         topk2_logps, topk2_inds = topk2_logps.topk(W, dim=-1)
    ...
    #         topk_beam_inds  = topk_inds.div(W)  # which beam is selected.
    #         topk_token_inds = topk2_inds.view(B, W * W).gather(1, topk_inds)  # W words selected (word + position)
    #         topk_word_inds  = topk_token_inds // (2 * t + 2)
    #         topk_pos_inds   = topk_token_inds  % (2 * t + 2)
    ...
    #         # alpha re-weight
    #         logps = logps * (1 + (eos_yet.float()) * 1 / (t + 1)).pow(alpha)
    #         eos_yet = eos_yet | (topk_word_inds == self.fields[field].vocab.stoi['<eos>'])
